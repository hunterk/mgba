...
static void _reloadSettings(void) {
	...
	if (environCallback(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
		if (strcmp(var.value, "Game Boy") == 0) {
			...
			model = GB_MODEL_CGB;
		} else if (strcmp(var.value, "Super Game Boy Color (SGB + GBC)") == 0) {
			model = GB_MODEL_SCGB;
		...
		mCoreConfigSetDefaultValue(&core->config, "scgb.model", modelName);
		mCoreConfigSetDefaultValue(&core->config, "agb.model", modelName);
	}
